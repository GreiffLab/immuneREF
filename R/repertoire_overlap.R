...
repertoire_overlap <- function(list_repertoires, basis="CDR3_aa"){
  ...
  rep_names <- names(list_repertoires)
  ...
  if(basis=="CDR3_aa"){
    overlap_tmp <- sapply(split(name_combo, col(name_combo)),
                          function(x) intersect_percentage_function(unique(as.character(list_repertoires[[x[1]]]$junction_aa)),unique(as.character(list_repertoires[[x[2]]]$junction_aa))))
  }else if(basis == "CDR3_nt"){
    overlap_tmp <- sapply(split(name_combo, col(name_combo)),
                          function(x) intersect_percentage_function(unique(as.character(list_repertoires[[x[1]]]$junction)),unique(as.character(list_repertoires[[x[2]]]$junction))))
  }else if(basis == "VDJ_aa"){
    overlap_tmp <- sapply(split(name_combo, col(name_combo)),
                          function(x) intersect_percentage_function(unique(as.character(list_repertoires[[x[1]]]$sequence_aa)),unique(as.character(list_repertoires[[x[2]]]$sequence_aa))))
  }else if(basis == "VDJ_nt"){
    overlap_tmp <- sapply(split(name_combo, col(name_combo)),
                          function(x) intersect_percentage_function(unique(as.character(list_repertoires[[x[1]]]$sequence)),unique(as.character(list_repertoires[[x[2]]]$sequence))))
  ...
  overlap_stat <- matrix(1, ncol = length(list_repertoires), nrow = length(list_repertoires))
