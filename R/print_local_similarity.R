#' Prints violin plots of local similarity values per layer, across categories
#'
...
#' @examples
#' \dontrun{
#' print_local_similarity(list_similarity_matrices,categories_list,path_figure="figures")
#' }


print_local_similarity<-function(list_similarity_matrices,categories_list,path_figure="figures"){

  categories_df<-categories_list[["categories"]]
  color_categories<-categories_list[["color"]]
  subsets <- categories_list[["subset"]]

  list_of_maxmin_rep<-list()
  for(i in 1:length(list_similarity_matrices)){
    ...
    for(z in 1:length(unique_subsets)){
      ...
      curr_graph <- igraph::graph.adjacency(curr_matrix, mode="undirected", weighted=TRUE)

      local_sim_df<-reshape2::melt(igraph::strength(curr_graph))
      names(local_sim_df)<-c("strength")
      local_sim_df$degree<-igraph::degree(curr_graph)
      local_sim_df$local_similarity<-local_sim_df$strength/sum(local_sim_df$strength)
      local_sim_df$file_name<-row.names(local_sim_df)
      row.names(local_sim_df)<-c(1:nrow(local_sim_df))
      local_sim_df$name<-unique_subsets[z]

      list_of_subsets[[z]]<-local_sim_df

      max_repertoire<-local_sim_df[local_sim_df$local_similarity==max(local_sim_df$local_similarity),"file_name"]
      min_repertoire<-local_sim_df[local_sim_df$local_similarity==min(local_sim_df$local_similarity),"file_name"]

      max_min_rep[[z]]<-c(max_rep=max_repertoire,min_rep=min_repertoire)
    }

    list_of_maxmin_rep[[current_layer]]<-max_min_rep
    bound_subsets<-do.call(rbind,list_of_subsets)


    plot_local_similarity <- ggplot2::ggplot(bound_subsets, ggplot2::aes(x=name,y=local_similarity,fill=name))+                    # basic graphical object
    ggplot2::geom_violin()+
    ggplot2::geom_boxplot(width=0.1, outlier.size = 0.25)+
    ggplot2::scale_fill_manual(values = color_categories)+
    ggplot2::labs(x="",y="Local similarity",fill="")+
    ggplot2::theme_bw()+
    ...
    grDevices::pdf(file.path(path_figure,paste("local_similarity_",current_layer,".pdf",sep="")), width = 7,  height = 5.5,onefile=FALSE)
    grid::pushViewport(grid::viewport(layout=grid::grid.layout(1,1), height=1,width=1)) # 3 rows, 1 columns
    vplayout<-function(x,y) grid::viewport(layout.pos.row=x,layout.pos.col=y)
    base::print(plot_local_similarity, vp=vplayout(1,1))   # plot for row 1, column 1
    grDevices::dev.off()
